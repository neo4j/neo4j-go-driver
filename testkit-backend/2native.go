...
func cypherToNative(c any) (any, error) {
	...
	case "CypherVector":
		...
		switch dtype {
		case "i8":
			vec := make(dbtype.Vector[int8], len(bytes))
			for i, b := range bytes {
				vec[i] = int8(b)
			}
			return vec, nil
		case "i16":
			...
			vec := make(dbtype.Vector[int16], len(bytes)/2)
			for i := 0; i < len(bytes); i += 2 {
				vec[i/2] = int16(binary.BigEndian.Uint16(bytes[i : i+2]))
			}
			return vec, nil
		case "i32":
			if len(bytes)%4 != 0 {
			...
			vec := make(dbtype.Vector[int32], len(bytes)/4)
			for i := 0; i < len(bytes); i += 4 {
				vec[i/4] = int32(binary.BigEndian.Uint32(bytes[i : i+4]))
			}
			return vec, nil
		case "i64":
			if len(bytes)%8 != 0 {
			...
			vec := make(dbtype.Vector[int64], len(bytes)/8)
			for i := 0; i < len(bytes); i += 8 {
				vec[i/8] = int64(binary.BigEndian.Uint64(bytes[i : i+8]))
			}
			return vec, nil
		case "f32":
			if len(bytes)%4 != 0 {
			...
			vec := make(dbtype.Vector[float32], len(bytes)/4)
			for i := 0; i < len(bytes); i += 4 {
				vec[i/4] = math.Float32frombits(binary.BigEndian.Uint32(bytes[i : i+4]))
			}
			return vec, nil
		case "f64":
			if len(bytes)%8 != 0 {
			...
			vec := make(dbtype.Vector[float64], len(bytes)/8)
			for i := 0; i < len(bytes); i += 8 {
				vec[i/8] = math.Float64frombits(binary.BigEndian.Uint64(bytes[i : i+8]))
			}
			return vec, nil
		default:
