...
var _ = Describe("Transaction", func() {
	...
	Context("V3", func() {
		...
		It("should set transaction timeout", func() {
			...
			session2, tx2 := newSessionAndTx(context.TODO(), driver, neo4j.AccessModeWrite)
			defer session2.Close()
			defer tx2.Close()

			updateNodeInTx(tx2, "TxTimeOut", map[string]interface{}{"id": 1})

			session3, tx3 := newSessionAndTx(context.TODO(), driver, neo4j.AccessModeWrite, neo4j.WithTxTimeout(1*time.Second))
