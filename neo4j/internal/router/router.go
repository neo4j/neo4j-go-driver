...
type Pool interface {
	...
	Borrow(ctx context.Context, servers []string, authToken map[string]any, wait bool, boltLogger log.BoltLogger, idlenessThreshold time.Duration) (db.Connection, error)
...
func (r *Router) readTable(ctx context.Context,
	...
	if dbRouter != nil && len(dbRouter.table.Routers) > 0 {
		...
		table, err = readTable(ctx, r.pool, routers, r.routerContext, bookmarks, database, impersonatedUser, sessionAuthToken, boltLogger)
	}
	...
	if table == nil {
		...
		table, err = readTable(ctx, r.pool, []string{r.rootRouter}, r.routerContext, bookmarks, database, impersonatedUser, sessionAuthToken, boltLogger)
	}
...
func (r *Router) getOrReadTable(ctx context.Context,
	...
	table, err := r.readTable(ctx, dbRouter, bookmarks, database, "", sessionAuthToken, boltLogger)
	if err != nil {
		return nil, err
	}
...
func (r *Router) Readers(ctx context.Context,
	bookmarks func(context.Context) ([]string, error),
	database string,
	sessionAuthToken map[string]any,
	boltLogger log.BoltLogger) ([]string, error) {

	table, err := r.getOrReadTable(ctx, bookmarks, database, sessionAuthToken, boltLogger)
	if err != nil {
		return nil, err
	}
...
func (r *Router) Writers(ctx context.Context,
	bookmarks func(context.Context) ([]string, error),
	database string,
	sessionAuthToken map[string]any,
	boltLogger log.BoltLogger) ([]string, error) {
...
func (r *Router) GetNameOfDefaultDatabase(ctx context.Context, bookmarks []string, user string, sessionAuthToken map[string]any, boltLogger log.BoltLogger) (string, error) {
	table, err := r.readTable(ctx, nil, bookmarks, db.DefaultDatabase, user, sessionAuthToken, boltLogger)
	if err != nil {
		return "", err
	}
