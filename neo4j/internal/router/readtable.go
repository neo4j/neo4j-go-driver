...
func readTable(ctx context.Context, pool Pool, routers []string, routerContext map[string]string, bookmarks []string,
	database, impersonateAs string, boltLogger log.BoltLogger) (*db.RoutingTable, error) {
	...
	for _, router := range routers {
		...
		table, err = conn.GetRoutingTable(routerContext, bookmarks, database, impersonateAs)
		pool.Return(conn)
		if err == nil {
			return table, nil
		}
