...
func TestBolt4(ot *testing.T) {
	...
	connectToServer := func(t *testing.T, serverJob func(srv *bolt4server)) (*bolt4, func()) {
		...
		c, err := Connect("serverName", tcpConn, auth, "007", nil, logger, nil)
		if err != nil {
			t.Fatal(err)
		}
	...
	ot.Run("Routing in hello", func(t *testing.T) {
		...
		bolt, err := Connect("serverName", conn, auth, "007", routingContext, logger, nil)
		AssertNoError(t, err)
		bolt.Close()
	})
	...
	ot.Run("No routing in hello", func(t *testing.T) {
		...
		bolt, err := Connect("serverName", conn, auth, "007", nil, logger, nil)
		AssertNoError(t, err)
		bolt.Close()
	})
