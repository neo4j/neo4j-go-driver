...
func TestConnect(ot *testing.T) {
	...
	ot.Run("Server rejects versions", func(t *testing.T) {
		...
		_, err := Connect(context.Background(), "servername", conn, auth, "007", nil, logger, nil, nil)
		AssertError(t, err)
	})
	...
	ot.Run("Server answers with invalid version", func(t *testing.T) {
		...
		boltconn, err := Connect(context.Background(), "servername", conn, auth, "007", nil, logger, nil, nil)
		AssertError(t, err)
		if boltconn != nil {
