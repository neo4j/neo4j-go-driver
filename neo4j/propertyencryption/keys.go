...
// ErrKeyNotFound is returned by an EncapsulatedKeyRecordRepository when no key is stored under
// the requested id or alias.
//
...
// EncapsulatedKey identifies a stored data encryption key.
//
...
// EncapsulatedKeyRecord is a stored key together with what is needed to recover it.
//
// EncapsulatedKeyRecord is part of the property encryption preview feature (see README on what
// it means in terms of support and compatibility guarantees).
type EncapsulatedKeyRecord struct {
	EncapsulatedKey
...
// KeyEncapsulationResult is a new data encryption key together with its protected form.
//
// KeyEncapsulationResult is part of the property encryption preview feature (see README on what
// it means in terms of support and compatibility guarantees).
type KeyEncapsulationResult struct {
	// Key is the data encryption key, which must be AES-256. The driver encrypts with it and
	// does not retain it.
	Key []byte
...
type KeyEncapsulationService interface {
	...
	Encapsulate(ctx context.Context, options map[string]string) (KeyEncapsulationResult, error)
...
// EncapsulatedKeyRecordRepository stores the data encryption keys an EnvelopeProfile uses.
// Keys are held in encapsulated form, so the repository never sees key material.
//
// Implementations are called while encrypting and decrypting, so they must be safe for
// concurrent use. Lookups should return ErrKeyNotFound when nothing matches.
//
// EncapsulatedKeyRecordRepository is part of the property encryption preview feature (see
// README on what it means in terms of support and compatibility guarantees).
type EncapsulatedKeyRecordRepository interface {
	// FindByID returns the key stored under id, or ErrKeyNotFound.
	FindByID(ctx context.Context, id string) (EncapsulatedKeyRecord, error)
